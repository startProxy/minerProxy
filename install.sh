...
*)
    echo -e " 
	 该 ${red}安装脚本${none} 不支持你的系统。 ${yellow}(-_-) ${none}
...
install_download() {
    ...
    git clone https://github.com/startProxy/minerProxy
...
start_write_config() {
    ...
    echo "下载完成，开启守护"
    echo
    chmod a+x $installPath/minerProxy_linux_web
    if [ -d "/etc/supervisor/conf/" ]; then
        rm /etc/supervisor/conf/minerProxy.conf -f
        echo "[program:minerProxy]" >>/etc/supervisor/conf/minerProxy.conf
        echo "command=${installPath}/minerProxy_linux_web" >>/etc/supervisor/conf/minerProxy.conf
        ...
        echo "command=${installPath}/minerProxy_linux_web" >>/etc/supervisor/conf.d/minerProxy.conf
        ...
        echo "command=${installPath}/minerProxy_linux_web" >>/etc/supervisord.d/minerProxy.ini
    ...
    echo "请以访问本机IP:18888"
    echo
    echo "安装完成...守护模式无日志，需要日志的请以nohup ./minerProxy_linux_web &方式运行"
    echo
    echo "配置文件：/etc/minerProxy/config.yml，网页端可修改登录密码token"
    echo
    echo "[*---------]"
    sleep 1
    echo "[**--------]"
...
while :; do
    ...
    echo "github下载地址:https://github.com/startProxy/minerProxy"
    echo ""
    echo
    echo " 1. 安装minerProxy"
    echo
